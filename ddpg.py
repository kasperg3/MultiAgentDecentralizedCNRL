...
class ReplayBuffer:
    ...
    def store(self, obs, act, rew, next_obs, done):
        _, _, state_next0, _ = core.unpackObs(obs)
        self.obs_buf[self.ptr] = state_next0
        _, _, state_next1, _ = core.unpackObs(next_obs)
        self.obs2_buf[self.ptr] = state_next1
...
def ddpg(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
         ...
         max_ep_len=1000, logger_kwargs=dict(), save_freq=1, render=False):
    """
    ...
    def get_action(o, noise_scale):
        _, _, state, _ = core.unpackObs(o)

        a = ac.act(torch.as_tensor(state, dtype=torch.float32))
        a += noise_scale * np.random.randn(action_dim)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='UrReach-v0')
